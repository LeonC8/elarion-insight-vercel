...
export async function GET(request: Request) {
  ...
  try {
    ...
    Object.keys(dataByMarketSegment).forEach((segment) => {
      if (dataByMarketSegment[segment].datasets.length_of_stay) {
        const existingBuckets = new Set(
          dataByMarketSegment[segment].datasets.length_of_stay.data.map(
            (item) => item.range
          )
        );
    ...
    Object.keys(dataByMarketSegment).forEach((segment) => {
      if (dataByMarketSegment[segment].datasets.length_of_stay) {
        dataByMarketSegment[segment].datasets.length_of_stay.data.sort(
          (a, b) => {
            return (
    ...
    return NextResponse.json(
      {
        error:
          "Failed to fetch length of stay distribution data by market segment from ClickHouse",
      },
      { status: 500 }
    );
  } finally {
