...
export async function GET(request: Request) {
  ...
  const primaryFilterField =
    searchParams.get("primaryFilterField") || "market_group_code"; // Field to filter on
  ...
  try {
    ...
    if (topCategoryOriginalValues.length > 0) {
      ...
      categoryFilter = `AND ${field} IN (${topCategoryOriginalValues
        .map((name) =>
    ...
    if (topCategoryOriginalValues.length > 0) {
      // Use original values in the IN clause, quoting strings properly
      prevCategoryFilter = `AND ${field} IN (${topCategoryOriginalValues
        .map((name) =>
          typeof name === "string" ? `'${name.replace(/'/g, "''")}'` : name
        )
        .join(",")})`;
