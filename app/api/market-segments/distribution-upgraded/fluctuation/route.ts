...
function generateCacheKey(params: URLSearchParams): string {
  ...
  relevantParams.forEach(key => {
    ...
    switch (key) {
        ...
        case 'primaryValue': value = params.get(key) || null; break;
        case 'primaryFilterField': value = params.get(key) || 'booking_channel'; break;
...
export async function GET(request: Request) {
  ...
  const primaryFilterField = searchParams.get('primaryFilterField') || 'booking_channel'; // Field to filter on
  ...
  try {
    ...
    // New: Add filter for specific primary value
    let primaryValueFilter = '';
    if (primaryValue) {
      primaryValueFilter = `AND ${primaryFilterField} = '${primaryValue}'`;
    }
