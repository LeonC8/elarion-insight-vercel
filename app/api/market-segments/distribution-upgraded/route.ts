...
function generateCacheKey(params: URLSearchParams): string {
  ...
  relevantParams.forEach(key => {
    ...
    switch (key) {
        ...
        case 'primaryField': value = params.get(key) || 'market_group_code'; break;
...
export async function GET(request: Request) {
  ...
  // --- CHECK CACHE ---
  const cacheKey = `market-segments:${generateCacheKey(searchParams)}`;
  ...
  try {
    ...
    // Get unique values for the primary field (market groups)
