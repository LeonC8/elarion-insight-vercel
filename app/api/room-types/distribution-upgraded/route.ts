...
function generateCacheKey(params: URLSearchParams): string {
  ...
  relevantParams.forEach(key => {
    ...
    switch (key) {
        ...
        case 'primaryField': value = params.get(key) || 'room_type'; break;
...
export async function GET(request: Request) {
  ...
  // --- CHECK CACHE ---
  const cacheKey = `room-types:${generateCacheKey(searchParams)}`;
  ...
  const primaryField = searchParams.get('primaryField') || 'room_type';
