...
export async function GET(request: Request) {
  ...
  try {
    ...
    occupancyPreviousData.forEach(prevItem => {
      ...
      const existingEntry = dataByRoomType[roomType].datasets.stay_date.data.find(
    ...
    bookingsPreviousData.forEach(prevItem => {
      ...
      const existingEntry = dataByRoomType[roomType].datasets.booking_date.data.find(
        item => item.range === dayName
      );
    ...
    Object.keys(dataByRoomType).forEach(roomType => {
      ...
      if (dataByRoomType[roomType].datasets.stay_date) {
        const existingDays = new Set(
          dataByRoomType[roomType].datasets.stay_date.data.map(item => item.range)
        );
      ...
      if (dataByRoomType[roomType].datasets.booking_date) {
        const existingDays = new Set(
          dataByRoomType[roomType].datasets.booking_date.data.map(item => item.range)
        );
    ...
    Object.entries(dataByRoomType).forEach(([roomType, roomTypeData]) => {
      ...
      const hasNonZeroBookingDate = roomTypeData.datasets.booking_date.data.some(
        item => item.current > 0 || item.previous > 0
      );
    ...
    return NextResponse.json(
      { error: 'Failed to fetch reservation trends data by room type from ClickHouse' },
      { status: 500 }
    );
  } finally {
