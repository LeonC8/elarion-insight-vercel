...
export async function GET(request: Request) {
  ...
  try {
    ...
    previousData.forEach(prevItem => {
      ...
      const existingEntry = dataByBookingChannel[bookingChannel].datasets.length_of_stay.data.find(
    ...
    Object.keys(dataByBookingChannel).forEach(channel => {
      if (dataByBookingChannel[channel].datasets.length_of_stay) {
        const existingBuckets = new Set(
          dataByBookingChannel[channel].datasets.length_of_stay.data.map(item => item.range)
        );
    ...
    Object.keys(dataByBookingChannel).forEach(channel => {
      if (dataByBookingChannel[channel].datasets.length_of_stay) {
        dataByBookingChannel[channel].datasets.length_of_stay.data.sort((a, b) => {
    ...
    Object.keys(dataByBookingChannel).forEach(channel => {
      if (channelNameMap[channel]) {
        dataByBookingChannel[channel].datasets.length_of_stay.title = `Length of Stay`;
      }
    });
    ...
    return NextResponse.json(
      { error: 'Failed to fetch length of stay distribution data by booking channel from ClickHouse' },
      { status: 500 }
    );
  } finally {
