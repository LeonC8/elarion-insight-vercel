...
export async function GET(request: Request) {
  ...
  try {
    ...
    occupancyPreviousData.forEach(prevItem => {
      ...
      const existingEntry = dataByBookingChannel[bookingChannel].datasets.stay_date.data.find(
    ...
    bookingsPreviousData.forEach(prevItem => {
      ...
      const existingEntry = dataByBookingChannel[bookingChannel].datasets.booking_date.data.find(
        item => item.range === dayName
      );
    ...
    Object.keys(dataByBookingChannel).forEach(channel => {
      ...
      if (dataByBookingChannel[channel].datasets.stay_date) {
        const existingDays = new Set(
          dataByBookingChannel[channel].datasets.stay_date.data.map(item => item.range)
        );
      ...
      if (dataByBookingChannel[channel].datasets.booking_date) {
        const existingDays = new Set(
          dataByBookingChannel[channel].datasets.booking_date.data.map(item => item.range)
        );
    ...
    return NextResponse.json(
      { error: 'Failed to fetch reservation trends data by booking channel from ClickHouse' },
      { status: 500 }
    );
  } finally {
