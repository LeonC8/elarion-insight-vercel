...
export async function GET(request: Request) {
  ...
  const reportDateParam =
    searchParams.get("reportDate") || format(new Date(), "yyyy-MM-dd");
  const viewType = (searchParams.get("viewType") || "Day") as
    | "Day"
    | "Month"
    | "Year";
  const comparison = (searchParams.get("comparison") || "Yesterday") as
    | "Yesterday"
    | "Last 7 days"
    | "Last 15 days"
    | "Last 30 days";
  ...
  try {
    client = createClient({
      ...
      database: "JADRANKA", // Specify database here
    });
    ...
    const currentOccupancyRange = getOccupancyDateRange(
      currentBookingDate,
      viewType
    );
    ...
    const currentData: QueryResult =
      currentDataRaw && currentDataRaw.length > 0
        ? (currentDataRaw[0] as QueryResult)
        : {
            total_sold_rooms: 0,
            total_room_revenue: 0,
            total_cancelled_rooms: 0,
            total_revenue_lost: 0,
          };

    const comparisonDataBase: QueryResult =
      comparisonDataRaw && comparisonDataRaw.length > 0
    ...
    if (comparison !== "Yesterday") {
      ...
      comparisonData = {
        ...
        total_cancelled_rooms:
          comparisonDataBase.total_cancelled_rooms / divisor,
    ...
    // --- Process Daily Data ---
    const currentDailyDataRaw = await currentDailyResult.json<
      DailyQueryResult[]
    >();
    ...
    const currentDailyMap = new Map(
      currentDailyDataRaw.map((d) => [d.occupancy_date, d])
    );
    ...
    if (comparison === "Yesterday") {
      ...
      // 'Last X days' - data is already averaged DailyAverageResult
    ...
    const adrFluctuation = createFluctuationData(
      ...
      (d) =>
        calculateAdr(
          d?.avg_daily_room_revenue ?? 0,
          d?.avg_daily_sold_rooms ?? 0
        )
    );
    ...
    const currentAdr = calculateAdr(
      currentData.total_room_revenue,
      currentData.total_sold_rooms
    );
    const comparisonAdr = calculateAdr(
      comparisonData.total_room_revenue,
      comparisonData.total_sold_rooms
    );
    ...
    const responseData: PickupKpiResponse = [
      ...
      },
      {
        ...
        prefix: "€",
        fluctuation: adrFluctuation.map((f) => ({
          // Round ADR fluctuation values
          ...f,
    ...
    return NextResponse.json(
      { error: "Failed to fetch pickup KPI data", details: errorMsg },
      { status: 500 }
    );
  } finally {
    await client?.close();
