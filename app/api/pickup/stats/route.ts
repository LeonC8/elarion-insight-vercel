...
export async function GET(request: Request) {
  ...
  if (!businessDateParam || !bookingDateParam || (!occupancyDateParam && !occupancyMonthParam)) {
    return NextResponse.json({ 
      error: 'businessDate, bookingDate, and either occupancyDate or occupancyMonth parameters are required' 
    }, { status: 400 });
  }
  ...
  try {
    ...
    if (occupancyDateParam) {
      ...
      occupancyClause = `toDate(occupancy_date) = '${formatDateUTC(new Date(occupancyDateParam))}'`;
      ...
      if (monthIndex === -1 || isNaN(year)) {
        return NextResponse.json({ error: 'Invalid occupancyMonth format. Expected "MMM YYYY"' }, { status: 400 });
      }
      
      const startOfMonth = formatDateUTC(new Date(Date.UTC(year, monthIndex, 1)));
      const endOfMonth = formatDateUTC(new Date(Date.UTC(year, monthIndex + 1, 0))); // Last day of month
    ...
    const prevYearBookingDate = formatDateUTC(new Date(
    ...
    const prevYearQuery = `
      ...
      FROM SAND01CN.insights
      WHERE
        ${occupancyClause.replace(/(\d{4})/g, (match) => (parseInt(match) - 1).toString())}
        AND scd_valid_from <= toDateTime('${bookingDate}')
        AND scd_valid_to > toDateTime('${bookingDate}')
    `;
    ...
    const stats: PickupStats = {
      ...
      roomsCancelled: Math.round(ensureNumber(typedCurrentStats.rooms_cancelled)),
      revenueLost: Math.round(ensureNumber(typedCurrentStats.revenue_lost)),
      roomsAvailable: Math.round(ensureNumber(typedCurrentStats.rooms_available)),
