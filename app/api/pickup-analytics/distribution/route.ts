...
function generateCacheKey(params: URLSearchParams): string {
  ...
  relevantParams.forEach((key) => {
    ...
    switch (key) {
      case "reportDate":
        value = params.get(key) || format(new Date(), "yyyy-MM-dd");
        break;
      case "viewType":
        value = params.get(key) || "Day";
        break;
      case "comparison":
        value = params.get(key) || "Yesterday";
        break;
      case "field":
...
export async function GET(request: Request) {
  ...
  if (cachedEntry && cachedEntry.expiresAt > Date.now()) {
    console.log(
      `[Cache HIT] Pickup Distribution: ${cacheKey.substring(0, 100)}...`
    );
    ...
    console.log(
      `[Cache EXPIRED] Pickup Distribution: ${cacheKey.substring(0, 100)}...`
    );
    await deleteCacheEntry(cacheKey);
  } else {
    console.log(
      `[Cache MISS] Pickup Distribution: ${cacheKey.substring(0, 100)}...`
    );
  }
  // --- END CACHE CHECK ---
  ...
  const currentOccupancyRange = getOccupancyDateRange(
    reportDateParam,
    viewType
  ); // Use reportDateParam for occupancy range start
  ...
  try {
    ...
    // --- Execute Queries ---
    const [currentResultSet, comparisonResultSet, timeSeriesResultSet] =
      await Promise.all([
        client.query({
          query: currentQuery,
    ...
    currentData.forEach((row) => {
      ...
      revenue.push({
        ...
        change: roundValue(currentRevenue - prevRevenue),
        code,
      });
      ...
      roomsSold.push({
        name,
        value: currentRooms,
        change: roundValue(currentRooms - prevRooms),
        code,
      });
      ...
      adr.push({
        name,
        value: currentAdr,
        change: roundValue(currentAdr - prevAdr),
        code,
      });
    });
    ...
    console.log(
      `[Cache SET] Stored pickup distribution data for key: ${cacheKey.substring(
        0,
        100
      )}...`
    );
    ...
    console.error(
      `Error fetching pickup distribution data for field '${field}':`,
      error
    );
    const errorMsg =
    ...
    return NextResponse.json(
      { error: `Failed to fetch pickup distribution data`, details: errorMsg },
      { status: 500 }
    );
  } finally {
    await client?.close();
