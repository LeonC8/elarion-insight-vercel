...
export async function GET(request: Request) {
  ...
  try {
    ...
    const revenueData: AnalysisData[] = (isProducerRoute ? currentData : currentData.slice(0, limit)).map(item => {
    ...
    const roomsSoldData: AnalysisData[] = (isProducerRoute ? currentData : currentData.slice(0, limit)).map(item => {
      const prevItem = previousDataMap.get(item.field_name) || {
        total_revenue: 0,
        rooms_sold: 0,
        room_revenue: 0
    ...
    const adrData: AnalysisData[] = (isProducerRoute ? currentData : currentData.slice(0, limit)).map(item => {
      const prevItem = previousDataMap.get(item.field_name) || {
        rooms_sold: 0,
        room_revenue: 0
      };
    ...
    // Do not cache errors
    const errorBody = { error: `Failed to fetch ${field} data from ClickHouse` };
