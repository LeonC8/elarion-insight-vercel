...
interface DailySoldRooms {
  date: string;
  rooms: number;
}

interface RoomsAccumulator {
  [key: string]: DailySoldRooms;
}
...
export async function GET(request: Request) {
  try {
    ...
    const soldRooms = filteredData.reduce((acc: RoomsAccumulator, row) => {
      ...
      acc[date].rooms += row.roomsSold;
      return acc;
    }, {});

    const response = Object.values(soldRooms)
      .sort((a, b) => a.date.localeCompare(b.date));

    return NextResponse.json(response);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to calculate sold rooms trends' },
      { status: 500 }
    );
  }
} 