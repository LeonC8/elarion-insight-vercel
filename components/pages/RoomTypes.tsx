...
import { ResponsiveContainer, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, Legend, Bar, Line, ComposedChart } from 'recharts'
...
type Tab = 'All' | 'Standard' | 'Deluxe' | 'Suite' | 'Executive';
...
const roomTypeTableData: RoomTypeTableData[] = [
  {
    roomType: "Standard",
    rooms: 150,
    totalRevenue: 35000,
...
const RoomTypesTable = ({ data, comparisonType }: { data: RoomTypeTableData[], comparisonType: string }) => {
  ...
  return (
    <div className="bg-white shadow-lg border border-gray-200 rounded-lg overflow-hidden">
      ...
      <div className="flex justify-between items-center bg-white px-6 py-4">
          <h2 className="text-lg font-semibold bg-white text-gray-800">All room types</h2>
        </div>
        <table className="min-w-full divide-y divide-gray-300 border-t border-gray-300">
          <thead className="bg-gray-50 uppercase">
            <tr >
              <th className="px-6 py-4 text-left text-sm font-medium text-gray-800">Room Type</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map((row, index) => (
              <tr key={index}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 border-r border-gray-300 text-sm">{row.roomType}</td>
...
// Add this mock data after your existing data constants
const roomTypeBarData: RoomTypeData[] = [
  { 
    id: "Standard",
    bookedAndStayed: 750,
    freeUpgrades: 250,
...
// Add this new render function before the return statement in RoomTypesDashboard
const renderRoomTypeBarChart = () => (
  <Card className="bg-white shadow-lg rounded-lg overflow-hidden w-full mt-8">
    <CardHeader>
      <CardTitle className="text-lg font-semibold text-gray-800">Room Types Overview</CardTitle>
    </CardHeader>
    <CardContent>
      <div style={{ height: 400 }}>
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            />
            <Bar yAxisId="left" dataKey="bookedAndStayed" stackId="a" fill="#60a5fa" name="Booked & Stayed" />
            <Bar yAxisId="left" dataKey="freeUpgrades" stackId="a" fill="#f59e0b" name="Free Upgrades" />
            <Bar yAxisId="left" dataKey="paidUpgrades" stackId="a" fill="#10b981" name="Paid Upgrades" />
            <Bar yAxisId="left" dataKey="downgrades" stackId="a" fill="#6366f1" name="Downgrades" />
            <Line 
              yAxisId="right"
              type="monotone" 
              dataKey="occupancy" 
...
export function RoomTypesDashboard() {
      ...
      <div className="p-8">
        ...
        <div className="flex flex-col mb-6">
          <div className="flex justify-between items-center">  
            <div className="flex items-center">
              <h2 className="text-3xl font-bold text-gray-800">Room Types</h2>
            </div>
          ...
          <div className="flex space-x-8 mt-6 border-b">
            {(['All', 'Standard', 'Deluxe', 'Suite', 'Executive'] as Tab[]).map((tab) => (
              <button
                key={tab}
        ...
        <div className="grid grid-cols-1 gap-8">
          {selectedTab === 'All' && (
            <RoomTypesTable data={roomTypeTableData} comparisonType={comparisonType} />
          )}
        </div>
        ...
        {/* Add the room type bar chart */}
        {selectedTab === 'All' && renderRoomTypeBarChart()}
      </div>
