...
export function TopFive({
  ...
  useEffect(() => {
    ...
    const shouldFetch =
      (isInView || !lazyLoad) &&
      (fetchedParams === null || fetchedParams !== currentParamsString);
  ...
  const metricsFromApi =
    apiData && apiEndpoint && fetchedParams === JSON.stringify(apiParams);
  ...
  const allData = [...currentData].sort((a, b) => {
    // Use the potentially updated currentData
    switch (filterType) {
      case "top":
  ...
  const showSkeleton =
    apiLoading || (apiEndpoint && lazyLoad && !fetchedParams && !apiError);
      ...
      >
        <CardHeader className="flex flex-col md:flex-row items-start justify-between pb-2">
          ...
          <div className="flex flex-col w-full md:w-auto">
            {" "}
            {/* Ensure title takes full width on mobile */}
        ...
        <CardContent className="pt-3">
          ...
          {showSkeleton && (
            <div className="space-y-4 pt-4">
              {" "}
              {/* Add pt-4 to match data row padding */}
              ...
              <div className="mt-6 pt-4 border-t border-transparent">
                {" "}
                {/* Match spacing, transparent border */}
                <div className="flex justify-end">
                  <Skeleton className="h-9 w-24" />{" "}
                  {/* Approx size of Button */}
                </div>
              </div>
            </div>
          )}
          ...
          {!showSkeleton &&
            !apiError &&
            (apiEndpoint
              ? fetchedParams === JSON.stringify(apiParams)
              : true) && (
              <>
                {/* Show actual data */}
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between py-4">
                          ...
                          <div className="flex items-center">
                            ...
                            <span className="flex items-center text-sm w-20 justify-end">
                              &nbsp;
                            </span>
                          </div>
                        </div>
                    ...
                    <TableBody>
                      ...
                      {allData.length > 0 ? (
                        allData.map((item, idx) => (
                          <TableRow
                            key={item.code || item.name || `data-${idx}`}
