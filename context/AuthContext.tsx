...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoading) {
      console.log('AuthProvider: Route protection check', { user: !!user, pathname });
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (result.success && result.token) {
        ...
        document.cookie = `firebase-token=${result.token}; path=/; max-age=3600; secure; samesite=strict`;
